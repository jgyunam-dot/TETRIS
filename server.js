...
function handleBingoMessage(id,msg){
  ...
  switch(msg.type){
    ...
    case 'bingo_join':{
      ...
      if(rm.phase==='playing'||rm.phase==='ended'){bingoSend(id,{type:'bingo_error',msg:'이미 게임 중인 방이에요!'});break;}
      if(Object.keys(rm.players).length>=5){bingoSend(id,{type:'bingo_error',msg:'방이 꽉 찼어요! (최대 5명)'});break;}
      clientBingoRoom[id]=c;
    ...
    case 'bingo_start':{
      if(!p||id!==room.hostId||room.phase==='playing'||room.phase==='ended')break;
      if(Object.keys(room.players).length<2){bingoSend(id,{type:'bingo_error',msg:'최소 2명이 필요해요!'});break;}
      ...
      room.turnOrder=Object.keys(room.players); // 입장 순서 고정
      bingoBroadcast(room,{type:'bingo_game_start',firstTurnId:room.currentTurnId,players:bingoPlayerList(room)});
      console.log(`[빙고] 게임 시작: ${room.code} (${room.turnOrder.length}명)`);
      break;
    }
    case 'bingo_call':{
      ...
      // 라운드 로빈 턴 전환
      room.currentTurnId=bingoNextTurn(room,id);
    ...
    case 'bingo_update_count':{
      ...
      // 모든 플레이어에게 누구의 카운트인지 포함해서 전송
      Object.keys(room.players).filter(pid=>pid!==id).forEach(pid=>{
        bingoSend(pid,{type:'bingo_opponent_bingo',count,fromId:id,fromName:p.name});
      });
      break;
    }
    ...
    case 'bingo_restart':{
      ...
      room.phase='lobby';room.calledNumbers=[];room.currentTurnId=null;room.turnOrder=[];
...
function bingoNextTurn(room, currentId){
  const alive=(room.turnOrder||Object.keys(room.players)).filter(pid=>room.players[pid]);
  if(alive.length===0)return currentId;
  const idx=alive.indexOf(currentId);
  if(idx!==-1){
    // 정상 턴 전환: 현재 플레이어가 아직 alive에 있음
    return alive[(idx+1)%alive.length];
  } else {
    // 나간 플레이어의 차례였던 경우: 저장해둔 원래 인덱스 사용
    const nextIdx=(room._leaverOrigIdx!=null)?(room._leaverOrigIdx%alive.length):0;
    room._leaverOrigIdx=null;
    return alive[nextIdx]||alive[0];
  }
}
...
function handleBingoLeave(id){
  ...
  const wasTurn=room.currentTurnId===id;

  // 나가는 사람의 원래 순서 인덱스를 미리 저장 (다음 사람 계산용)
  if(wasPlaying&&wasTurn&&room.turnOrder){
    const origIdx=room.turnOrder.indexOf(id);
    room._leaverOrigIdx=origIdx>=0?origIdx:null;
  }

  // turnOrder에서도 제거
  if(room.turnOrder) room.turnOrder=room.turnOrder.filter(pid=>pid!==id);
  ...
  if(Object.keys(room.players).length===0){
    ...
    if(wasPlaying){
      ...
      if(remaining<2){
        ...
        bingoBroadcast(room,{type:'bingo_win',winnerId,winnerName:room.players[winnerId]?.name});
      } else {
        // 게임 계속 - 나간 사람 차례였으면 다음 사람으로
        if(wasTurn) room.currentTurnId=bingoNextTurn(room,id);
        bingoBroadcast(room,{type:'bingo_player_left',leftId:id,nextTurnId:room.currentTurnId,hostId:room.hostId});
      }
    } else {
      // 로비에서 나간 경우
...
const server=http.createServer((req,res)=>{
  ...
  let filename;
  if(urlPath==='/'||urlPath===''||urlPath==='/menu'||urlPath==='/index.html') filename='index.html';
